...
def extract_first_quiz_answers(response_json):
    ...
    quizs= response_json["output"]["quiz_questions"]
    quiz1 = random.choice(list(quizs.values()))
    question = quiz1["question"]
    correct_key = next(key for key in quiz1.keys() if key.startswith("correct"))
    correct_answer = quiz1[correct_key]
    keys = list(quiz1.keys())
    keys.remove(correct_key)
    keys.remove("question")
    answer_key = keys[0]

    for i in range(len(quiz1[answer_key])):
        print(quiz1)
        if i == correct_answer:
            continue
        quiz_wrong.append(quiz1[answer_key][i])
        print(i)
        print(quiz1[answer_key][i])
    quiz_correct = quiz1[answer_key][correct_answer]
    ...
    print(shuffled_quiz_options)
    st.session_state["quiz_options"] = shuffled_quiz_options
